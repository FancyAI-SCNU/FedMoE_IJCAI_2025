...
def fedmoe_train(model, dataset, batch_size, optimizer, epoch, entropy_loss_weight, aux_weight, l2_weight, device_index,
    ...
    for e in range(epoch):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            optimizer.zero_grad()
            outputs, aux_loss, gates, _ = model(inputs)
            if e == epoch - 1:
            ...
            model_kl_mean += F.kl_div(outputs.log_softmax(dim=1), one_hot_labels.float().softmax(dim=1),
                                      reduction='batchmean')
    ...
    if entropy_loss_flag and cur_epoch != -1:
        scheduler.step()
        writer.add_scalar('train/kl_mean',
                          model_kl_mean / len(trainloader), cur_epoch)
        writer.add_scalar('train/total_loss',
                          model_total_loss / len(trainloader), cur_epoch)
        writer.add_scalar('train/CE_loss',
                          model_CE_loss / len(trainloader), cur_epoch)
        writer.add_scalar('train/model_aus_loss',
                          model_aux_loss / len(trainloader), cur_epoch)
        writer.add_scalar('train/l2_loss',
                          model_l2_loss / len(trainloader), cur_epoch)
        writer.add_scalar('train/model_entropy_loss',
                          model_entropy_loss / len(trainloader), cur_epoch)
...
def server_gate(model, fedavgloader, device_index):
    ...
    with torch.no_grad():
        for data in fedavgloader:
            inputs, labels = data
            inputs, labels = inputs.to(device), labels.to(device)
            outputs, _, gates, _ = model(inputs)
            for _, (gate, label) in enumerate(zip(gates, labels)):
                tensor[:, label.item()] += gate.squeeze()
